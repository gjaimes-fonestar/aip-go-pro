...
export function installBrowserPolyfill(): void {
    ...
    // Fan-out sets so multiple components can subscribe concurrently
    const deviceSubs  = new Set<(json: string) => void>()
    const channelSubs = new Set<(ev: AipChannelPlayerEvent) => void>()

    aip.onDeviceEvent((json: string) => {
        deviceSubs.forEach((cb) => cb(json))
    })

    aip.onChannelEvent((json: string) => {
        try {
            const ev = JSON.parse(json) as AipChannelPlayerEvent
            channelSubs.forEach((cb) => cb(ev))
